...
where we had parametrized $Z_j{}^{s,r,\ell }(h)= a \text{Cos}\left[\frac{1}{s}\{\text{rh}+2\pi (j-1)\}\right]+ i\frac{b}{\ell }\text{Sin}\left[\frac{\ell }{s}\{\text{rh}+2\pi (j-1)\}\right]$. The semiholomorphic map $f\left(u,v,\bar{v}\right)$ with knotted zero line is fond by the replacements, in $p_h^{s,r,\ell}(u)$, $\exp (i h)\leftarrow v$}v$ and $\exp (-i h)\leftarrow\bar{v}$ . The mapping to the complex plane can be obtained by considering the substitutions 
